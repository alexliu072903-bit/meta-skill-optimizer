...
from common import WORKSPACE, file_digest, load_json, require_empty_or_missing
...
def import_manifest(manifest_path: Path, destination: Path) -> dict:
    ...
    try:
        ...
        for item, source, required in resolved:
            ...
            if required and source.exists() and record["file_count"] == 0:
                raise SystemExit(f"Required source selected no files: {source_id}")
            locked_sources.append(record)

        lock = {"version": 1, "subject_id": manifest["id"], "sources": locked_sources}
        (temporary / "subject.lock.json").write_text(
...
def main() -> None:
    ...
    lock = import_manifest(args.manifest, args.destination)
